<script setup>
const user = inject("user");

const startingColumns = ["yearStart", "yearEnd", "campus"]; // Key of starting columns in list of all columns

const listOfAllColumns = [
    {
        key: "yearStart",
        label: "Starting Year",
        sortable: true,
    },
    {
        key: "yearEnd",
        label: "Ending Year",
        sortable: true,
    },
    {
        key: "campus",
        label: "Campus",
        sortable: true,
    },
    {
        key: "id",
        label: "ID",
        sortable: true,
    },
    {
        key: "createdAt",
        label: "Entry Created",
        sortable: true,
    },
    {
        key: "updatedAt",
        label: "Entry Updated",
        sortable: true,
    },
    {
        key: "currentSchoolYear",
        label: "Current School Year",
        sortable: true,
    },
];

// Editing Mode
const editModeIsOpen = ref(false);
function enableEditMode() {
    editModeIsOpen.value = true;
    colorEditable.value = "red";
}
function discardChanges() {
    editModeIsOpen.value = false;
    colorEditable.value = "gray";
    tableRef.value.closeDataModal();
    tableRef.value.updateTable();
}

async function updateChanges() {
    await useFetch("/api/admin/editForms/manageSY/update", {
        method: "POST",
        body: JSON.stringify(selectedData.value),
    });
    editModeIsOpen.value = false;
    colorEditable.value = "gray";
    tableRef.value.closeDataModal();
    tableRef.value.updateTable();
}

const selectedData = ref();
function selectedRow(data) {
    selectedData.value = data;
    selectedData.value.createdAt = dayjs(data.createdAt).format(
        "MMM DD, YYYY ; HH:mm",
    );
    selectedData.value.updatedAt = dayjs(data.updatedAt).format(
        "MMM DD, YYYY ; HH:mm",
    );
}

// Only allow editing if the user is an administrator
const allowedEditing = ref(user.role === "ADMIN");

// Editing Mode Appearances
const colorEditable = ref("gray");

const tableRef = ref();

// Add Record Modal
const newItemData = ref({ currentSchoolYear: false });
const addRecordModalIsOpen = ref(false);
function closeAddRecordModal() {
    newItemData.value = { currentSchoolYear: false };
    addRecordModalIsOpen.value = false;
    tableRef.value.closeAddRecord();
}
async function addNewItem() {
    await useFetch("/api/admin/editForms/manageSY/add", {
        method: "POST",
        body: JSON.stringify(newItemData.value),
    });

    newItemData.value = { currentSchoolYear: false };
    tableRef.value.closeAddRecord();
    tableRef.value.updateTable();
}

const failedDeletion = ref(false);
async function deleteItem() {
    editModeIsOpen.value = false;
    colorEditable.value = "gray";
    const deletion = await useFetch("/api/admin/editForms/manageSY/delete", {
        method: "POST",
        body: JSON.stringify(selectedData.value),
    });

    if (deletion.data.value === "DELETION FAILED; MARKED HIDDEN") {
        failedDeletion.value = true;
    }

    tableRef.value.closeDataModal();
    tableRef.value.updateTable();
}

// Delete All Requests Modal
const deleteAllRequestsModalIsOpen = ref(false);
function openDeleteAllRequestsModal() {
    deleteAllRequestsModalIsOpen.value = true;
}
function closeDeleteAllRequestsModal() {
    deleteAllRequestsModalIsOpen.value = false;
}
async function deleteAllRequestsFromSchoolYear() {
    const deletionRequests = await useFetch(
        "/api/admin/editForms/manageSY/deleteAllRequests",
        {
            method: "POST",
            body: JSON.stringify(selectedData.value),
        },
    );
    console.log(deletionRequests);
    closeDeleteAllRequestsModal();
    tableRef.value.closeDataModal();
}
</script>

<template>
    <div>
        <!-- Failed Deletion Modal -->
        <UModal v-model="failedDeletion">
            <UCard>
                <template #header>
                    <span class="text-red-500">LOCATION DELETION FAILED</span>
                </template>
                <p>
                    Failed to delete {{ selectedData.name }} due to its
                    connection to existing data. Thus its forced deletion may
                    cause unwanted changes in other records.
                    <br />
                    <br />
                    <span class="font-bold">
                        It has instead been marked as hidden and will not be
                        seen by anyone except admins.
                    </span>
                </p>
            </UCard>
        </UModal>

        <!-- Delete all Requests from the School Year Modal -->
        <UModal v-model="deleteAllRequestsModalIsOpen">
            <UCard>
                <template #header
                    >DELETE ALL REQUESTS FROM SY {{ selectedData.yearStart }} -
                    {{ selectedData.yearEnd }}
                </template>
                <p>
                    Are you sure you want to delete all requests related to
                    {{ selectedData.yearStart }} - {{ selectedData.yearEnd }}?
                    This includes all laboratory requests, equipment requests,
                    material requests, and reagent requests. We recommend saving
                    a backup in the "Settings > Save Backup".
                </p>
                <template #footer>
                    <div class="flex w-auto justify-between">
                        <UButton
                            label="CANCEL"
                            variant="outline"
                            color="green"
                            @click="closeDeleteAllRequestsModal"
                        />
                        <UButton
                            label="DELETE ALL REQUESTS"
                            color="primary"
                            @click="deleteAllRequestsFromSchoolYear"
                        />
                    </div>
                </template>
            </UCard>
        </UModal>

        <!-- Data Table -->
        <TablesWSlottedModals
            ref="tableRef"
            title="SCHOOL YEARS"
            default-sort-key="yearStart"
            :starting-columns="startingColumns"
            :list-of-all-columns="listOfAllColumns"
            fetch-path="/api/db/rawData/getAllSY"
            :allowed-editing="allowedEditing"
            :edit-mode-is-open="editModeIsOpen"
            @selected-row="selectedRow"
        >
            <template #dataModal>
                <UCard>
                    <template #header>
                        <div
                            class="flex flex-row items-center justify-between space-x-3"
                        >
                            <h1>
                                {{ selectedData.name }}
                            </h1>
                        </div>
                    </template>
                    <div
                        v-for="(item, index) in listOfAllColumns"
                        :key="item.key"
                        class="grid grid-cols-3 gap-1 gap-y-2"
                    >
                        <!-- Left Label -->
                        <div
                            v-if="!['id'].includes(item.key)"
                            class="flex items-center p-1"
                        >
                            {{ item.label }}
                        </div>

                        <!-- Right Current Value -->
                        <div
                            v-if="
                                ![
                                    'id',
                                    'campus',
                                    'createdAt',
                                    'updatedAt',
                                    'currentSchoolYear',
                                ].includes(item.key)
                            "
                            class="col-span-2 p-1"
                        >
                            <UInput
                                v-model="selectedData[item.key]"
                                type="number"
                                min="1"
                                oninput="validity.valid||(value='');"
                                :color="colorEditable"
                                :disabled="!editModeIsOpen"
                                variant="outline"
                            />
                        </div>

                        <!-- Exceptions to UInput Fields -->
                        <div v-if="item.key == 'campus'" class="col-span-2 p-1">
                            <UInput
                                v-model="selectedData[item.key]"
                                :color="colorEditable"
                                :disabled="!editModeIsOpen"
                                variant="outline"
                            />
                        </div>
                        <div
                            v-if="
                                item.key == 'createdAt' ||
                                item.key == 'updatedAt'
                            "
                            class="col-span-2 p-1"
                        >
                            <UInput
                                v-model="selectedData[item.key]"
                                color="gray"
                                :disabled="true"
                                variant="outline"
                            />
                        </div>

                        <div
                            v-if="item.key == 'currentSchoolYear'"
                            class="col-span-2 p-1"
                        >
                            <v-switch
                                v-model="selectedData[item.key]"
                                :readonly="!editModeIsOpen"
                                color="primary"
                                hide-details
                            />
                        </div>

                        <!-- Delete Entry Button -->
                        <div
                            v-if="
                                editModeIsOpen &&
                                index == listOfAllColumns.length - 1
                            "
                        ></div>
                        <div
                            v-if="
                                editModeIsOpen &&
                                index == listOfAllColumns.length - 1
                            "
                            class="col-span-2"
                        >
                            <UButton
                                label="Delete School Year"
                                variant="outline"
                                color="red"
                                @click="deleteItem"
                            />
                        </div>

                        <!-- Delete All Data related to that school year -->
                        <div
                            v-if="
                                editModeIsOpen &&
                                index == listOfAllColumns.length - 1
                            "
                            class="col-span-3 mt-3 flex flex-col items-center justify-center text-center"
                        >
                            <h1 class="text-red-500">DANGER ZONE</h1>
                            <p>
                                The button below will delete all requests
                                related to that school year but not the school
                                year itself as it may still be connected to
                                database transactions. This includes all
                                laboratory requests, equipment requests,
                                material requests, and reagent requests. We
                                recommend saving a backup in the "Settings >
                                Save Backup".
                            </p>
                            <UButton
                                class="my-5"
                                label="Delete ALL REQUESTS FROM School Year"
                                variant="outline"
                                color="red"
                                @click="openDeleteAllRequestsModal"
                            />
                        </div>
                    </div>
                    <template #footer>
                        <div class="flex w-auto justify-center">
                            <UButton
                                v-if="!editModeIsOpen"
                                color="green"
                                label="EDIT"
                                icon="i-material-symbols-edit"
                                @click="enableEditMode"
                            />
                            <div v-else class="flex w-[100%] justify-between">
                                <UButton
                                    variant="outline"
                                    icon="i-material-symbols-cancel"
                                    color="red"
                                    label="CANCEL"
                                    @click="discardChanges"
                                />
                                <UButton
                                    variant="outline"
                                    icon="i-material-symbols-save"
                                    label="SAVE"
                                    color="green"
                                    @click="updateChanges"
                                />
                            </div>
                        </div>
                    </template>
                </UCard>
            </template>
            <template #addRecordModal>
                <UCard>
                    <template #header>
                        <div class="flex items-center justify-between">
                            <h1>ADD A NEW ITEM</h1>
                        </div>
                    </template>

                    <div
                        v-for="item in listOfAllColumns"
                        :key="item.key"
                        class="grid grid-cols-3 gap-1 gap-y-2"
                    >
                        <!-- Left Label -->
                        <div
                            v-if="
                                !['id', 'createdAt', 'updatedAt'].includes(
                                    item.key,
                                )
                            "
                            class="flex items-center p-1"
                        >
                            {{ item.label }}
                        </div>

                        <!-- Right Current Value -->
                        <div
                            v-if="
                                ![
                                    'id',
                                    'currentSchoolYear',
                                    'createdAt',
                                    'updatedAt',
                                    'campus',
                                ].includes(item.key)
                            "
                            class="col-span-2 p-1"
                        >
                            <UInput
                                v-model="newItemData[item.key]"
                                type="number"
                                min="1"
                                oninput="validity.valid||(value='');"
                                :color="colorEditable"
                                variant="outline"
                                placeholder="Required"
                            />
                        </div>

                        <!-- Exceptions to UInput -->
                        <div v-if="item.key == 'campus'" class="col-span-2 p-1">
                            <UInput
                                v-model="newItemData[item.key]"
                                variant="outline"
                                placeholder="Required"
                                :color="colorEditable"
                            />
                        </div>
                        <div
                            v-if="item.key == 'currentSchoolYear'"
                            class="col-span-2 p-1"
                        >
                            <v-switch
                                v-model="newItemData[item.key]"
                                color="primary"
                                hide-details
                            />
                        </div>
                    </div>

                    <template #footer>
                        <div class="flex w-[100%] justify-between">
                            <UButton
                                variant="outline"
                                icon="i-material-symbols-cancel"
                                color="red"
                                label="CANCEL"
                                @click="closeAddRecordModal"
                            />
                            <UButton
                                variant="outline"
                                icon="i-material-symbols-save"
                                label="SAVE"
                                color="green"
                                :disabled="
                                    !newItemData.yearStart ||
                                    !newItemData.yearEnd ||
                                    !newItemData.campus
                                "
                                @click="addNewItem"
                            />
                        </div>
                    </template>
                </UCard>
            </template>
        </TablesWSlottedModals>
    </div>
</template>
